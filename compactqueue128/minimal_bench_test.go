...
func BenchmarkPushBursty(b *testing.B) {
	...
	rand.Seed(42)
	baseTick := int64(64) // Start in second group
	...
	for i := 0; i < b.N; i++ {
		...
		if i%1000 == 0 {
			baseTick = (baseTick + 32) % (BucketCount - 32)
		}
	}
}
...
func BenchmarkUnlinkMin_StableBucket(b *testing.B) {
	...
	q.Push(64, h, testValue) // Use tick in second group
	b.ResetTimer()
...
func BenchmarkUnlinkMin_DenseBucket(b *testing.B) {
	...
	for i := 0; i < 3; i++ {
		q.Push(64, hs[i], testValue) // Use tick in second group
	}
	b.ResetTimer()
...
func BenchmarkSharedPoolMultiQueue(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		case 0:
			...
			q1.Push(int64(i%100), h, testValue)
		case 1:
			h := Handle(i%1000 + 2000) // Range 2000-2999
			q2.Push(int64(i%100), h, testValue)
		case 2:
			h := Handle(i%1000 + 3000) // Range 3000-3999
			q3.Push(int64(i%100), h, testValue)
		}
	}
}
...
func BenchmarkPoolUtilization(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		q.Push(int64(i%100), h, testValue)
	}
}
