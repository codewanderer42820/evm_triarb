...
// Zero-allocation connection implementations
...
// Pre-allocated test data - computed once, reused everywhere
var (
	// Ethereum-like payloads of various sizes (pre-allocated)
	...
	// Pre-built frames (header + payload)
	frame64    []byte
	frame512   []byte
	frame1536  []byte
	frame4096  []byte
	...
	// Fragmented frame sequences
	fragmented4096  []byte
	fragmented16384 []byte
	fragmented65536 []byte

	// Control frame mixed sequences
	controlMixed []byte
...
// Fast frame builder - no allocations
...
// Generate realistic Ethereum JSON-RPC data in-place
...
func init() {
	...
	// Pre-build fragmented sequences
	buildFragmentedSequence()
	buildControlMixedSequence()
	buildWorstCaseSequences()
}

func buildFragmentedSequence() {
	var buf [70000]byte
	pos := 0

	// 4096 bytes in 512-byte fragments
	...
	// 16384 bytes in 1460-byte fragments (MTU-sized)
...
func buildControlMixedSequence() {
	...
	// data, ping, data, pong, data pattern
...
func buildWorstCaseSequences() {
	...
	// Reset for alternating sequence
	pos = 0

	// Alternating control/data frames
	for i := 0; i < 20; i++ {
		// Control frame
...
// Helper to create test frames with allocations (for unit tests only)
...
func BenchmarkFragmentation(b *testing.B) {
	tests := []struct {
		...
		{"1KB_max_frag", maxFragmented, 1024},
	}
...
func BenchmarkControlFrames(b *testing.B) {
	...
	b.SetBytes(1536 * 3) // 3 data frames
	b.ReportAllocs()
	b.ResetTimer()

	for i := 0; i < b.N; i++ {
		conn.reset()
		// Process 3 data frames (control frames handled internally)
		SpinUntilCompleteMessage(conn)
		SpinUntilCompleteMessage(conn)
		SpinUntilCompleteMessage(conn)
	}
...
func BenchmarkLengthEncoding(b *testing.B) {
	tests := []struct {
		...
		{"7bit_125B", frame64, 64},
		{"16bit_1536B", frame1536, 1536},
		{"64bit_65KB", frame65536, 65536},
	}
