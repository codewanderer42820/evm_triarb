...
// PeepMin32 returns up to 32 handles in ascending-tick order without modifying
// the queue. It performs all work on the stack, using tombstone bitmasks to mark
// consumed buckets, lanes, and groups. This is zero-alloc and O(32) = O(1).

//go:inline
//go:nosplit
func (q *QuantumQueue) PeepMin32() [32]Handle {
	// 1) Initialize result array to all nilIdx via exponential copy
	var res [32]Handle
	res[0] = nilIdx
	for width := 1; width < 32; width <<= 1 {
		copy(res[width:], res[:width])
	}

	// Tombstone masks track which groups, lanes, and buckets we've consumed
	var (
		groupMask  uint64                        // 1 = group fully consumed
		laneMask   [GroupCount]uint64            // per-group mask of consumed lanes
		bucketMask [GroupCount][LaneCount]uint64 // per-lane mask of buckets
	)

	count := 0
	var nextDup Handle = nilIdx

	// 2) Loop until we fill res or run out of entries
	for count < 32 {
		// A) If more items in the current bucket chain, drain them first
		if nextDup != nilIdx {
			h := nextDup
		...
		// B) Find next live group not yet tombstoned
		...
		// G) Mark bucket consumed
		bucketMask[g][l] |= 1 << (63 - t)
		// H) If lane exhausted, tombstone lane
		if q.groups[g].l2[l]&^bucketMask[g][l] == 0 {
			laneMask[g] |= 1 << (63 - l)
			// I) If group exhausted, tombstone group
