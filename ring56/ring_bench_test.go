...
//
// Performance expectations (realistic for Go with 56-byte payloads):
//   - Single-threaded Push: 25-120ns per operation
//   - Single-threaded Pop: 25-120ns per operation
//   - Push+Pop pair: 50-240ns per operation
//   - Cross-core SPSC: 120-600ns per operation (including coordination overhead)
//   - Sustained throughput: 4-40M ops/sec depending on pattern and core count
...
func BenchmarkRegression_RealisticThresholds(b *testing.B) {
	...
	// Realistic performance thresholds for Go with 56-byte payloads
	if avgLatency > 600 { // 600ns per push+pop pair (adjusted for larger payload)
		b.Errorf("Performance regression: %.1f ns/op (threshold: 600ns)", avgLatency)
	}
	if throughput < 1_600_000 { // 1.6M ops/sec minimum (adjusted for larger payload)
		b.Errorf("Throughput regression: %.0f ops/sec (threshold: 1.6M)", throughput)
	}
...
func BenchmarkRing_CacheBehavior(b *testing.B) {
	// Test various sizes to understand cache impact with 56-byte payloads
	sizes := []int{
		16,    // Fits in L1 cache
		32,    // Still L1 cache
		64,    // L2 cache territory
		256,   // L2/L3 cache territory
