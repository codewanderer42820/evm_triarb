...
//   - Sub-60ns operation latency for Push/Pop operations
...
func testData(seed byte) *[56]byte {
	data := &[56]byte{}
	for i := range data {
		data[i] = seed + byte(i)
	}
...
func validateData(t *testing.T, got, want *[56]byte, context string) {
	t.Helper()
	if got == nil && want != nil {
...
// TestSlotAlignment validates 64-byte slot alignment for cache efficiency
func TestSlotAlignment(t *testing.T) {
	r := New(4)

	// Verify each slot is properly aligned to 64-byte boundaries
	for i := 0; i < len(r.buf); i++ {
		slotAddr := uintptr(unsafe.Pointer(&r.buf[i]))
		if slotAddr%64 != 0 {
			t.Errorf("slot %d at %p not aligned to 64-byte boundary", i, &r.buf[i])
		}
	}

	// Verify slot size is exactly 64 bytes
	slotSize := unsafe.Sizeof(slot{})
	if slotSize != 64 {
		t.Errorf("slot size = %d bytes, want 64 bytes", slotSize)
	}
}
