...
// Memory Layout:
//
//	64-byte alignment ensures efficient cache utilization with one slot
//	per 64-byte cache line, maximizing memory bandwidth usage and minimizing
//	cache line contention between adjacent slots.
//
//go:notinheap
//go:align 64
type slot struct {
	val [56]byte // Fixed payload size optimized for larger message passing
